...
async function getRecipesByFirstLetter(req: Request, res: Response) {
  const {q: query} = req.query;
  const url = req.baseUrl;
  if (!query || typeof query !== 'string' || query.length !== 1 || !(/[a-zA-Z]/).test(query as string)) {
    return res.status(400).json({ error: 'Invalid input' });
  }
  try {
    const recipes = await NameService.getRecipesByFirstLetter(url as EndpointRequest, query as string);
    return res.status(200).json(recipes);
  } catch (error) {
    return res.status(500).json({ error: 'Internal server error' });
  }
